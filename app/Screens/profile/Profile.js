...
import { View, Text, SafeAreaView, Image, TouchableOpacity, Share, Animated, Platform, ActivityIndicator, RefreshControl } from 'react-native';
...
const Profile = ({ navigation }) => {
    ...
    const renderAdItem = (data, isPending = true) => {
        const imageUrl = data.picture?.url?.medium || 
        ...
        return (
            ...
            >
                <View style={{ flexDirection: 'row', borderBottomWidth: isPending ? 1 : 0, borderBottomColor: colors.border, paddingBottom: isPending ? 10 : 0 }}>
                    <Image
                    ...
                    <View style={{ marginLeft: 10, flex: 1 }}>
                        <Text numberOfLines={1} style={{ ...FONTS.fontSm, ...FONTS.fontSemiBold, color: colors.title }}>
                            {data.title || 'Untitled Listing'}
                        </Text>
                        <Text style={{ ...FONTS.font, ...FONTS.fontMedium, color: colors.title, marginTop: 2 }}>
                        ...
                        {isPending ? (
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 5 }}>
                                ...
                                <Text style={[FONTS.fontXs, { fontSize: 11, color: colors.text, marginLeft: 4 }]}>
                                    {data.city_id ? `City ID: ${data.city_id}` : 'Location not specified'}
                                </Text>
                            </View>
                        )}
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 10, paddingBottom: 0 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 15 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                                ...
                                <Text style={{ ...FONTS.fontXs, color: colors.title }}>{data.visits || 0}</Text>
                            </View>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                                ...
                                <Text style={{ ...FONTS.fontXs, color: colors.title }}>{data.likes || 0}</Text>
                            </View>
                        </View>
                        <TouchableOpacity 
                            onPress={() => moresheet.current.openSheet(data)}
                            style={[GlobalStyleSheet.background, { marginRight: 5, height: 40, width: 40 }]}
                        >
                            <Image
            ...
            >
                    ...
                    <View style={[GlobalStyleSheet.container, { paddingBottom: 10 }]}>
                        ...
                        <View style={[GlobalStyleSheet.shadow2, { borderWidth: 0, backgroundColor: COLORS.primary, marginTop: 20, borderRadius: 20 }]}>
                            ...
                            <View style={{ backgroundColor: COLORS.secondary, flex: 1, padding: 20, borderRadius: 20, alignItems: 'center', borderTopLeftRadius: 25, borderTopRightRadius: 25 }}>
                                ...
                                <Text style={[FONTS.font, { color: COLORS.white, marginTop: 5, opacity: .7 }]}>{userData?.email ? userData.email : 'anonymous@example.com'}</Text>
                                <View style={{ backgroundColor: COLORS.white, paddingTop: 5, borderRadius: 9, marginTop: 15, flexDirection: 'row', gap: 20, paddingHorizontal: 20 }}>
                                    <TouchableOpacity style={{ alignItems: 'center' }} onPress={() => navigation.navigate('FollowerFollowing')}>
                                    ...
                                    <LinearGradient colors={['rgba(0, 0, 0, 0.0)', 'rgba(18, 9, 46, 0.20)', 'rgba(0, 0, 0, 0.0)']} style={{ width: 2, height: 50 }}></LinearGradient>
                                    <TouchableOpacity style={{ alignItems: 'center' }} onPress={() => navigation.navigate('FollowerFollowing')}>
                                        <Text style={{ ...FONTS.h6, ...FONTS.fontMedium, color: COLORS.title }}>360</Text>
                                        <Text style={{ ...FONTS.fontRegular, fontSize: 12, color: COLORS.title, opacity: .7, lineHeight: 14 }}>Favorites</Text>
                                    </TouchableOpacity>
                                </View>

                                <TouchableOpacity style={{ backgroundColor: COLORS.primary, padding: 10, borderRadius: 50, position: 'absolute', top: 10, right: 10 }} onPress={() => navigation.navigate('EditProfile')}>
                                    <Image
                ...
                <SafeAreaView style={{ backgroundColor: colors.card }}>
                    ...
                    {loading ? (
                        ...
                        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', height: 200 }}>
                            <Text style={{ color: colors.title }}>Error loading ads: {error}</Text>
                            <TouchableOpacity onPress={fetchAds} style={{ marginTop: 10, padding: 10, backgroundColor: COLORS.primary, borderRadius: 5 }}>
                                        ...
                                        <View style={{ paddingHorizontal: 10, flex: 1, paddingBottom: 80 }}>
                                            {pendingAds.length > 0 ? (
                                                ...
                                                <View style={{ alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                                                    <Text style={{ color: colors.text }}>No pending ads found</Text>
                                                </View>
                                            )}
                                        </View>
                                    </ScrollView>
                                    ...
                                    <ScrollView showsVerticalScrollIndicator={false}>
                                        <View style={{ paddingHorizontal: 10, flex: 1, paddingBottom: 80 }}>
                                            {archivedAds.length > 0 ? (
                                                ...
                                                <View style={{ alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                                                    <Text style={{ color: colors.text }}>No archived ads found</Text>
                                                </View>
                                            )}
                                        </View>
                                    </ScrollView>
