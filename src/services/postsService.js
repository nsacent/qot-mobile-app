...
export const listingTypesService = {
    ...
    getById: (id) => api.get(replaceParams(API_ENDPOINTS.POSTS_TYPES.BY_ID, { id })),
};

export const reportTypesService = {
    getAll: () => api.get(API_ENDPOINTS.POSTS_REPORT_TYPES.GET_ALL),
    getById: (id) => api.get(replaceParams(API_ENDPOINTS.POSTS_REPORT_TYPES.BY_ID, { id })),
};
...
export const postsService = {
    ...
    getPendingApproval: (page = 1, perPage = 12) =>
        postsService.getAll({ pendingApproval: 1, belongLoggedUser: 1, page, perPage }),

    getArchived: (page = 1, perPage = 12) =>
        postsService.getAll({ belongLoggedUser: 1, archived: 1, page, perPage }),

    getById: (id, embed = 'pictures') =>
        api.get(replaceParams(API_ENDPOINTS.POSTS.BY_ID, { id }), { params: { embed } }),
